...
def summarize_housing_loss(
    ...
    geoid_df = data_df.groupby('geoid').count()[['value']].rename(columns={'value':'total'})

    #pivot table to get count by geoid and type
    geoid_df_type = pd.pivot_table(data_df, index=['geoid'], columns='data_type', values = 'value', aggfunc = 'size', fill_value=0)

    #pivot table to get count of records by geoid, data_type and year
    geoid_df_type_year = pd.pivot_table(data_df, index=['geoid'], columns='data_type_year', values = 'value', aggfunc = 'size', fill_value=0)

    #merge the tables
    geoid_df = geoid_df.merge(geoid_df_type, left_index=True, right_index=True)
    geoid_df = geoid_df.merge(geoid_df_type_year, left_index=True, right_index=True)
    ...
    geoid_df = geoid_df.merge(pop_df, left_index=True, right_on='GEOID', how='left')
