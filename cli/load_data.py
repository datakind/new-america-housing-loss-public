...
def main(input_path: str, update_status_db: callable = None) -> None:
    ...
    # Create the directories to output the correlation plots to
    # rename Type to be lower case if it exists
    if 'Type' in df.columns:
        df.rename(columns={'Type': 'type'}, inplace=True)
    for target_var in df['type'].unique():
        type_output_path = Path(input_path).parent / OUTPUT_PATH_PLOTS / target_var
    ...
    write_df_to_disk(
        df_geocoded_final, geocoded_file_write_path / GEOCODED_FILENAME
    )

    #ACS DATA
    # Get the state and county fips to pull ACS data for
    state_fips, county_fips, city_str, state_str = find_state_county_city(
        df_geocoded_final
    )

    # ACS data pull used in housing loss summary and demographic correlation search
    ...
    if df_geocoded_final is not None:
        df_geo_errors = df_geocoded_final.merge(hhs, left_on='geoid', right_on='GEOID', how = 'left', indicator=True)
        df_geo_errors = df_geo_errors[df_geo_errors['_merge'] == 'left_only'][['street_address_1', 'city', 'state', 'zip_code']]
        df_geo_errors['errors'] = 'Unable to find a match in the census geocoder'

    #COMBINE AND OUTPUT ERRORS
    if (df_parse_err is not None or df_geo_errors is not None or df_dups is not None):
        ...
        #drop if street_address_1 is null
        df_errors = df_errors[df_errors['street_address_1'].notnull()]
        write_df_to_disk(
            df_errors, summary_write_path / ADDRESS_ERR_FILENAME
        )

    # CREATE HOUSING LOSS SUMMARIES
    summ = summarize_housing_loss(df_geocoded_final, hhs)

    # MERGE ACS DATA WITH SUMMARY DATA
    ...
    for target_var in df['type'].unique():
        try:
            correlation_analysis(acs_df, df_summ_mrg, target_var, type_output_path)
        except KeyError:
            print('Unable to create correlations for ' + target_var)
